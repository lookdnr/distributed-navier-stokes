...
void calculate_ppm_RHS_central(Domain &PPrhs, Domain &u, Domain &v,
                               const Domain &obstacle, const SubDomain &sd) {
  ...
  for (int i = i_start; i <= i_end; i++)
    for (int j = j_start; j <= j_end; j++) {
      ...
      // check neighbours: zero the velocity on boundaries, else use normal
      // stencil
      ...
      PPrhs(i, j) =
          rho / dt *
          ((u_right - u_left) / (2. * dx) + (v_up - v_down) / (2. * dy));
    }
}
    ...
    for (int i = i_start; i <= i_end; i++)
      for (int j = j_start; j <= j_end; j++) {
        ...
        const double P_right =
            (obstacle(i + 1, j) > 0.5) ? P_old(i, j) : P_old(i + 1, j);
        const double P_left =
            (obstacle(i - 1, j) > 0.5) ? P_old(i, j) : P_old(i - 1, j);
        const double P_up =
            (obstacle(i, j + 1) > 0.5) ? P_old(i, j) : P_old(i, j + 1);
        const double P_down =
            (obstacle(i, j - 1) > 0.5) ? P_old(i, j) : P_old(i, j - 1);

        // use the above in the stencil:
        P(i, j) = 1.0 / (2.0 + 2.0 * (dx * dx) / (dy * dy)) *
                  (P_left + P_right + (P_up + P_down) * (dx * dx) / (dy * dy) -
                   (dx * dx) * PPrhs(i, j));
...
double project_velocity(Domain &u, Domain &v, Domain &P, const Domain &obstacle,
                        const SubDomain &sd) {
  const int nx = sd.nx_local;
  const int ny = sd.ny_local;
  // compoute local start indices based on grid pos
...
void solve_NS(Domain &P, Domain &P_old, Domain &PPrhs, Domain &u, Domain &u_old,
              Domain &v, Domain &v_old, Domain &obstacle, const SubDomain sd) {
  double vel_max = 0.0;
  int time_it = 0;
  int its;
  int out_it = 0;
...
int main(int argc, char *argv[]) {
  ...
  Domain obstacle =
      extension::build_square_obstacle(subdomain, half_width, Nx, Ny);
  ...
  utils::exchange_halos(
      obstacle, subdomain); // the obstacle is fixed, so just exchange halo once
