...
void write_solver_parameters(const string root="./out/extension"){
	// create dir if it doesnt exist
		if (!filesystem::exists(root)) filesystem::create_directories(root);
...
void grids_to_file(int timestep,
	const Domain& P, 
	const Domain& u, 
	const Domain& v, 
	const SubDomain& sd,
	const bool debug_write=false,
	const string root="./out/extension"
)
{
	...
	if (debug_write) {nx++; ny++; i_min--; j_min--;}
	...
	<< "$ t = " << t << '\n';
	for (int i = i_min; i <= nx; i++)	// note here and all other loops we skip the halo
	{
...
void calculate_ppm_RHS_central(Domain& PPrhs, Domain& u, Domain& v, const Domain& obstacle, const SubDomain& sd)
{
	...
	for (int i = i_start; i <= i_end; i++)
		...
		{
			if (obstacle(i, j) > 0.5) {
				PPrhs(i, j) = 0.0; // skip building the RHS inside the obstacle
				continue;
			}

			// check neighbours: zero the velocity on boundaries, else use normal stencil
			const double u_right = (obstacle(i + 1, j) > 0.5) ? 0.0 : u(i + 1, j);
			const double u_left = (obstacle(i - 1, j) > 0.5) ? 0.0 : u(i - 1, j);
			const double v_up = (obstacle(i, j + 1) > 0.5) ? 0.0 : v(i, j+1);
			const double v_down = (obstacle(i, j - 1) > 0.5) ? 0.0 : v(i, j-1);

			PPrhs(i, j) = rho / dt * ((u_right - u_left) / (2. * dx) + (v_up - v_down) / (2. * dy));
		}
}
...
int pressure_poisson_jacobi(Domain& P, 
	Domain& P_old, 
	Domain& PPrhs, 
	const Domain& obstacle,
	const SubDomain sd, 
	double rtol = 1.e-5)
{
	...
	while (tol > rtol)
	{
		...
		for (int i = i_start; i <= i_end; i++)
			for (int j = j_start; j <= j_end; j++)
			{
                // do not solve pressure inside the obstacle
                if (obstacle(i, j) > 0.5) {
                    P(i, j) = P_old(i, j);
                    continue;
                }

				// we need to check if neighbours are an obstacle cell
				// if neighbour is solid, use this cells pressure to enforce no slip
				const double P_right = (obstacle(i + 1, j) > 0.5) ? P_old(i, j) : P_old(i+1, j);
				const double P_left = (obstacle(i - 1, j) > 0.5) ? P_old(i, j) : P_old(i-1, j);
				const double P_up = (obstacle(i, j + 1) > 0.5) ? P_old(i, j) : P_old(i, j+1);
				const double P_down = (obstacle(i, j - 1) > 0.5) ? P_old(i, j) : P_old(i, j-1);

				// use the above in the stencil:
				P(i, j) = 1.0 / (2.0 + 2.0 * (dx * dx) / (dy * dy)) * (P_left + P_right + 
					(P_up + P_down) * (dx * dx) / (dy * dy)
					- (dx * dx) * PPrhs(i, j));
...
void calculate_intermediate_velocity(Domain& u, 
	Domain& u_old, 
	Domain& v, 
	Domain& v_old, 
	Domain& obstacle,
	const SubDomain& sd)
{
	...
	for (int i = i_start; i <= i_end; i++)
		...
		//safety pass: ensure velocities are zeroed
    for (int i = 1; i <= nx; ++i) {
        for (int j = 1; j <= ny; ++j) {
            if (obstacle(i, j) > 0.5) {
                u(i, j) = 0.0;
...
double project_velocity(Domain& u, Domain& v, Domain& P, const Domain& obstacle, const SubDomain& sd)
{
	const int nx = sd.nx_local;
	const int ny = sd.ny_local;
	// compoute local start indices based on grid pos
...
)
{
	...
	while (t < t_end)
	{
		...
		calculate_intermediate_velocity(u, u_old, v, v_old, obstacle, sd);

		// exchange halos since updated u and v required for ppm RHS
		utils::exchange_halos(u, sd);
		utils::exchange_halos(v, sd);

		calculate_ppm_RHS_central(PPrhs, u, v, obstacle, sd);
		its = pressure_poisson_jacobi(P, P_old, PPrhs, obstacle, sd, 1.e-4);
		vel_max = project_velocity(u, v, P, obstacle, sd);
		...
		if (t >= t_out)
		{
			...
			t_out += dt_out;
			if (id == 0) {
				cout << "Iteration " << time_it << ": t = " << t << "\n\tJacobi iterations: " << its << " vel_max: " << vel_max << endl;
				cout.flush();
			}
			grids_to_file(out_it, P, u, v, sd);
		}
...
{   
	...
	for (int i = 1; i < argc; ++i) {
		...
		if ((arg == "--Nx" || arg == "-nx") && i + 1 < argc) { Nx = std::stoi(argv[++i]); }
		else if ((arg == "--Ny" || arg == "-ny") && i + 1 < argc) { Ny = std::stoi(argv[++i]); }
		else if ((arg == "--Lx" || arg == "-lx") && i + 1 < argc) { Lx = std::stod(argv[++i]); }
		else if ((arg == "--Ly" || arg == "-ly") && i + 1 < argc) { Ly = std::stod(argv[++i]); }
		else if (arg == "-h" || arg == "--help") {
			if (id == 0) std::cout << "Usage: [--Nx N] [--Ny N] [--Lx val] [--Ly val]" << std::endl;
			MPI_Finalize();
			return 0;
		}
	}
    ...
    vector<SubDomain> decomp_metadata; // to contain decomp instructions for scatter
    ...
    if (id == 0) {
	    ...
	    if (p > 1) utils::print_metadata(decomp_metadata); // print computed decomp metadata
    }

	// scatter metadata
	SubDomain subdomain;
	MPI_Scatter(decomp_metadata.data(), 1, MPI_SubDomain, &subdomain, 1, MPI_SubDomain, 0, MPI_COMM_WORLD);
	...
	// build obstacles into each domain
	Domain obstacle = extension::build_square_obstacle(subdomain, half_width, Nx, Ny);
	...
	utils::exchange_halos(obstacle, subdomain); // the obstacle is fixed, so just exchange halo once

	solve_NS(P, P_old, PPrhs, u, u_old, v, v_old, obstacle, subdomain);

	if (id == 0) cout << endl << "Solve complete." << endl;
